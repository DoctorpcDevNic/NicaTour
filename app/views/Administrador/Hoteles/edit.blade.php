...
@section('contenido')
	...
	<div class="container-fluid">
		<h2 class="titul">Hoteles de {{$depto->nombre}}</h2>		
				@if(!count($Hotels)==0)					
					<div class="row">
						<div class="col-md-4">
						...
						<div class="col-md-8">
							<h2 class="titul">{{$Hotels->nombre}}</h2>
						</div>
					</div>				
			<div class="row">
					<div class="divider">
			...
			{{--Editar Informacion General--}}
						...
						{{Form::open(array('url'=>'administrador/Hoteles/Update/General/'.$Hotels->id, 'class' => 'form-horizontal'))}}
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-7">
								{{ Form::text('municipio', Input::old('titulo') ? Input::old():$Hotels->municipio, array('class' => 'form-control', 'placeholder'=> 'Municipio')) }}
							</div>
						</div>
						<div class="form-group">
							{{ Form::label('Nombre', 'Nombre', array('class' => 'col-sm-2 control-label')) }}
							<div class="col-sm-7">
								{{ Form::text('nombre', Input::old('titulo') ? Input::old():$Hotels->nombre, array('class' => 'form-control', 'placeholder'=> 'Nombre del local')) }}
							</div>
						</div>
						<div class="form-group">
							{{ Form::label('telefono', 'Telefono', array('class' => 'col-sm-2 control-label')) }}
							<div class="col-sm-7">
								{{ Form::text('telefono', Input::old('telefono') ? Input::old():$Hotels->telefono, array('class' => 'form-control', 'placeholder'=> 'Telefono del local')) }}
							</div>
						</div>
						<div class="form-group">
							{{ Form::label('direccion', 'Direccion', array('class' => 'col-sm-2 control-label')) }}
							<div class="col-sm-7">
								{{ Form::textarea('direccion', Input::old('direccion') ? Input::old():$Hotels->direccion, array('class' => 'form-control', 'placeholder'=> 'Direccion del local', 'size' => '30x5')) }}
							</div>
						</div>
						<div class="form-group">
								...
								{{--Recorre todas las descripciones y compara si pertenece al mismo idioma para editarlo--}}
									@if($key->id!=1)
										@if($valor->id_idioma==$key->id)
											{{Form::open(array('url'=>'administrador/Hoteles/Update/Traduccion/'.$valor->id, 'class' => 'form-horizontal'))}}
												<div class="form-group">
										...
										{{--Se muestran los idiomas que no tienen taduccion--}}
											@if((count($idioma)-1)>count($Descripcion))
												<div class="divider">
													{{Form::open(array('url'=>'administrador/Hoteles/traduccion/add/'.$key->id.'/'.$Hotels->id, 'class' => 'form-horizontal'))}}
														<div class="form-group">
															{{ Form::label('Idioma', $key->nombre, array('class' => 'col-sm-6 control-label')) }}
														</div>
														<div class="form-group">
				...
				{{--Si no existe ninguna traduccion, permite agregar exceptuando el espanol--}}
							@else
								@foreach($idioma as $variable)
									@if($variable->id!=1)
										<div class="divider">
											{{Form::open(array('url'=>'administrador/Hoteles/traduccion/add/'.$variable->id.'/'.$Hotels->id, 'class' => 'form-horizontal'))}}
												<div class="form-group">
