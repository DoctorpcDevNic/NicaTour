				...
				@foreach($Rest as $key)					
					<div class="row">
						<div class="col-md-3 " style="background:red;">
							<img class="img-responsive hidden-xs" src="{{ asset( 'img/restaurante-01.png') }}" alt="">
						</div>
						<div class="col-md-9">									
...
<script type="text/javascript">
       function realizaProceso(nombre){
        if(nombre!=""){
        	...
        	$.ajax({
                data:  parametros,
                url:   '{{ URL::to('/administrador/Restaurantes/Buscar') }}',
                type:  'post',
                beforeSend: function () {
                ...
                success:  function (data) {
                    ...
                    var alerta='<div class="alert alert-info" role="alert">'+ data.datos.length+' '+ data.sms+' "'+data.resultado+'"</div>';
                        for (datas in data.datos) {
                          ...
                          dhtml+='		<img class="img-responsive hidden-xs" src="{{ asset( 'img/restaurante-01.png') }}" alt="">';
                          dhtml+='	</div>';
